...
//       // Venue information - can be location object or URL
//       venue_info,
...
//     // Process venue_info based on is_virtual flag
//     let processedVenueInfo;
//     const isVirtualEvent = is_virtual === 'true' || is_virtual === true;

//     if (!venue_info) {
//       if (isVirtualEvent) {
//         return res.status(400).json({ error: "Virtual events require venue_info with URL" });
//       } else {
//         return res.status(400).json({ error: "Offline events require venue_info with address and coordinates" });
//       }
//     }

//     // Process venue_info
//     if (isVirtualEvent) {
//       // For virtual events, venue_info should be a URL
//       processedVenueInfo = venue_info;
//     } else {
//       // For offline events, venue_info should be a location object
//       try {
//         // If it's a string, try to parse it as JSON
//         if (typeof venue_info === 'string') {
//           processedVenueInfo = JSON.parse(venue_info);
//         } else {
//           processedVenueInfo = venue_info;
//         }
        
//         // Convert back to JSON string for storage
//         processedVenueInfo = JSON.stringify(processedVenueInfo);
//       } catch (error) {
//         return res.status(400).json({ 
//           error: "Invalid venue_info format. For offline events, provide location object with address and coordinates." 
//         });
//       }
//     }

//     // Validate venue_info using EventSchema validation
//     const tempEventData = {
//       venue_info: processedVenueInfo,
//       is_virtual: isVirtualEvent
//     };
    
//     const validationErrors = EventSchema.validateEventData(tempEventData);
...
//         await EventSchema.addHostBanner(event.id, req.files.hostBanner[0].buffer);
//       }
...
//     // Format response with parsed venue info
//     const venueInfo = EventSchema.parseVenueInfo(createdEvent.venue_info, createdEvent.is_virtual);
    
//     const responseEvent = {
//       ...createdEvent,
//       venue_info: venueInfo, // Parsed venue info for frontend
//       map_url: !createdEvent.is_virtual ? EventSchema.getMapUrl(createdEvent.venue_info) : null, // Map URL for offline events
//       event_images: createdEvent.event_images ? createdEvent.event_images.length : null,
//       host_photos: createdEvent.host_photos ? createdEvent.host_photos.length : null,
//       host_banner: createdEvent.host_banner ? 1 : 0, // Single banner count
//       host_gallery: createdEvent.host_gallery ? createdEvent.host_gallery.length : null // Gallery images count
//     };
...
//       // Process host banner (single image) - FIXED METHOD NAME
//       if (req.files.hostBanner && req.files.hostBanner.length > 0) {
//         console.log('Processing host banner'); // Debug log
//         await EventSchema.setHostBanner(event.id, req.files.hostBanner[0].buffer);
//       }
...
//     // FIXED RESPONSE FORMAT - Show actual image URLs/data instead of just counts
//     const responseEvent = {
//       ...createdEvent,
//       venue_info: venueInfo, // Parsed venue info for frontend
//       map_url: !createdEvent.is_virtual ? EventSchema.getMapUrl(createdEvent.venue_info) : null, // Map URL for offline events
...
//       event_images: createdEvent.event_images ? createdEvent.event_images.map((img, index) => ({
//         id: index,
//         url: `data:image/jpeg;base64,${img.toString('base64')}`
//       })) : [],
      
//       host_photos: createdEvent.host_photos ? createdEvent.host_photos.map((img, index) => ({
//         id: index,
//         url: `data:image/jpeg;base64,${img.toString('base64')}`
//       })) : [],
      
//       host_banner: createdEvent.host_banner ? {
//         url: `data:image/jpeg;base64,${createdEvent.host_banner.toString('base64')}`
//       } : null,
      
//       host_gallery: createdEvent.host_gallery ? createdEvent.host_gallery.map((img, index) => ({
//         id: index,
//         url: `data:image/jpeg;base64,${img.toString('base64')}`
//       })) : []
//     };
...
//         return {
//           ...eventData,
//           // FIXED: Return actual image URLs instead of just counts
//           event_images: (event.event_images && event.event_images.length > 0) ? 
...
//           host_social: {
//             // FIXED: Return actual image URLs for host photos
//             photos: (event.host_photos && event.host_photos.length > 0) ? 
...
//           // FIXED: Return actual banner URL
//           host_banner: event.host_banner ? {
//             url: `data:image/jpeg;base64,${event.host_banner.toString('base64')}`
//           } : null,

//           // FIXED: Return actual gallery URLs
//           host_gallery: (event.host_gallery && event.host_gallery.length > 0) ? 
...
//     // FIXED: Format event for response with actual image URLs
//     const formattedEvent = {
//       ...event,
//       event_images: event.event_images ? event.event_images.map((img, index) => ({
//         id: index,
//         url: `data:image/jpeg;base64,${img.toString('base64')}`
//       })) : [],
      
//       host_photos: event.host_photos ? event.host_photos.map((img, index) => ({
//         id: index,
//         url: `data:image/jpeg;base64,${img.toString('base64')}`
//       })) : [],
...
//       host_gallery: event.host_gallery ? event.host_gallery.map((img, index) => ({
//         id: index,
//         url: `data:image/jpeg;base64,${img.toString('base64')}`
//       })) : []
//     };
...
//       event_date,
//       event_time,
//       venue_info, // FIXED: Changed from 'location' to 'venue_info'
//       event_tags,
//       is_virtual,
//       is_free,
//       ticket_price,
...
//     if (event_date !== undefined) updateData.event_date = event_date;
//     if (event_time !== undefined) updateData.event_time = event_time;
//     if (venue_info !== undefined) updateData.venue_info = venue_info; // FIXED
//     if (is_virtual !== undefined) updateData.is_virtual = is_virtual === 'true' || is_virtual === true;
//     if (is_free !== undefined) {
//       updateData.is_free = is_free === 'true' || is_free === true;
//       updateData.ticket_price = updateData.is_free ? 0 : parseFloat(ticket_price) || 0;
...
//     if (seating !== undefined) updateData.seating = seating;
//     if (layout !== undefined) updateData.layout = layout;
//     if (language !== undefined) updateData.language = language; // FIXED: was setting layout instead of language
//     if (pet_allowance !== undefined) updateData.pet_allowance = pet_allowance;
//     if (age_limit !== undefined) updateData.age_limit = age_limit;
...
//       // FIXED: Handle host banner upload
//       if (req.files.hostBanner && req.files.hostBanner.length > 0) {
//         await EventSchema.setHostBanner(eventId, req.files.hostBanner[0].buffer);
//       }

//       // FIXED: Handle host gallery upload
//       if (req.files.hostGallery && req.files.hostGallery.length > 0) {
//         for (const file of req.files.hostGallery) {
...
//     // FIXED: Format response with actual image URLs
//     const responseEvent = {
//       ...finalEvent,
//       event_images: finalEvent.event_images ? finalEvent.event_images.map((img, index) => ({
//         id: index,
//         url: `data:image/jpeg;base64,${img.toString('base64')}`
//       })) : [],
      
//       host_photos: finalEvent.host_photos ? finalEvent.host_photos.map((img, index) => ({
//         id: index,
//         url: `data:image/jpeg;base64,${img.toString('base64')}`
//       })) : [],

//       host_banner: finalEvent.host_banner ? {
//         url: `data:image/jpeg;base64,${finalEvent.host_banner.toString('base64')}`
//       } : null,

//       host_gallery: finalEvent.host_gallery ? finalEvent.host_gallery.map((img, index) => ({
//         id: index,
//         url: `data:image/jpeg;base64,${img.toString('base64')}`
//       })) : []
//     };
...
//     // FIXED: Return actual image URLs
//     const formattedEvents = events.map(event => ({
//       ...event,
//       event_images: event.event_images ? event.event_images.map((img, index) => ({
//         id: index,
  ...
  try {
    ...
    if (!location || location.trim() === '') {
      return res.status(400).json({ 
        ...
        error: isVirtualEvent ? "Please enter HTTPS URL" : "Please enter location"
      });
    }
    ...
    if (isVirtualEvent) {
      ...
      if (!isValidHttpsUrl(location)) {
        return res.status(400).json({ error: "Please enter a valid HTTPS URL for virtual events" });
      }
    } else {
      // For offline events, just ensure location is not empty (any string is valid)
      if (location.trim().length < 3) {
        return res.status(400).json({ error: "Please enter a valid location for offline events" });
      }
    }
...
router.put('/updateEvent/:id', authenticate, upload.fields([
  ...
  try {
    ...
    if (location !== undefined) {
      const isVirtualEvent = is_virtual !== undefined ? (is_virtual === 'true' || is_virtual === true) : existingEvent.is_virtual;
